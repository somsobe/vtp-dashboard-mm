...
def extract_date_from_name(name: str) -> date:
    ...
    raise ValueError(
        f"Không lấy được ngày hợp lệ từ tên file: {name}"
    )
...
def clean_xuatsach(raidich_path: Path, ketnoi_path: Path, mapping_route_path: Path):
    SELECT_COLS = [
        ...
        "trong_luong","type","loai_dv","tg_nhap_buucuc","tg_laixe_nhan","deadline",
    ...
    def prep(p: Path):
        ...
        df = (
            ...
            .with_columns([
                ...
                .str.strptime(pl.Datetime, format="%Y-%m-%d %H:%M:%S", strict=False).alias(c)
                for c in TIME_COLS
            ])
            .with_columns([
...
def clean_15s(file_path: Path) -> pl.DataFrame:
    ...
    OUT_COLS = [
        "date", "Đơn vị", "Chuyến xe", "Tuyến xe", "Biển kiểm soát",
        "TG nhập tải đầu", "Tải kiện đầu", "TG nhập tải cuối", "Tải kiện cuối",
        "Số tải kiện", "TG quy định", "Số tải kiện đúng", "Số tải kiện sai",
    ]
    return(
        df
            .filter(pl.col("Chuyến xe") != "N/A")
            .with_columns(
                pl.col("Thời gian nhận bàn giao")
                .str.to_datetime("%d/%m/%Y %H:%M:%S", strict=False),
        
                pl.col("Thời gian quy định")
                .str.to_datetime("%d/%m/%Y %H:%M:%S", strict=False),
            )
            .with_columns(pl.col("Ngày báo cáo").str.strip_chars().str.to_date(format="%Y-%m-%d").alias("date"))
            .group_by(group_cols)
            .agg(
                pl.col("Thời gian nhận bàn giao")
                .drop_nulls()
                .min()
                .alias("TG nhập tải đầu"),
        
                pl.col("Mã tải kiện")
                .filter(pl.col("Thời gian nhận bàn giao").is_not_null())
                .sort_by("Thời gian nhận bàn giao")
                .first()
                .alias("Tải kiện đầu"),
                pl.col("Thời gian nhận bàn giao")
                .drop_nulls()
                .max()
                .alias("TG nhập tải cuối"),
                ...
                .alias("TG quy định"),
                pl.col("Mã tải kiện")
                .filter(pl.col("Đánh giá") == "Đúng giờ")
                .drop_nulls()
                .n_unique()
                .alias("Số tải kiện đúng"),
                pl.col("Mã tải kiện")
                .filter(pl.col("Đánh giá") == "Không đúng giờ")
                .drop_nulls()
                .n_unique()
                .alias("Số tải kiện sai"),
            )
            # .with_columns(pl.lit(file_date).cast(pl.Date).alias("date"))
            .select(OUT_COLS)
            .sort(group_cols)
    )


def append_clean_15s(df_new: pl.DataFrame, out_path: Path) -> pl.DataFrame:

    group_cols = ["date", "Đơn vị", "Chuyến xe", "Tuyến xe", "Biển kiểm soát"]
    if out_path.exists():
        df_old = pl.read_csv(out_path, try_parse_dates=True).with_columns(pl.col("date").cast(pl.Date))
        df_all = pl.concat([df_old, df_new], how="vertical_relaxed")
    else:
        df_all = df_new
    df_all = df_all.unique(subset=group_cols, keep="last").sort(group_cols)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    df_all.write_csv(out_path)
    return df_all
...
def clean_kndd(file_path: Path, lookup_route_path: Path, lookup_dvc_path: Path) -> pl.DataFrame:
    ...
    lookup_buucuc = pl.read_excel(r"D:\0. Library\dim_buucuc_tinhcu.xlsx")
    return (
        df.select(SELECT_COLS)
        .join(lookup_buucuc, left_on = "Mã điểm kết nối",right_on = "ma_buucuc",how = "left")
        .filter(pl.col("ma_tinh").is_not_null())
        .drop(["ma_tinh"])
        .filter(
...
def agg_xuatsach(df, lb):
    return (df.select(["don_vi_khaithac","ma_phieugui","date","danh_gia"])
        .rename({"don_vi_khaithac":"unit"}).group_by(["date","unit"])
        ...
        .join(lb.filter(pl.col("status")=="active").select(["unit","branch"]),on="unit",how="left").select(AGG_COLS).filter(pl.col("branch").is_not_null()))

def agg_15s(df, lb):
    """
    df: file clean lũy kế 15s_{month}.csv (mỗi dòng = 1 chuyến xe).
    numerator = tổng Số tải kiện đúng, denominator = tổng Số tải kiện, nhóm theo date × unit.
    """
    return (
        df.rename({"Đơn vị": "unit"})
        .group_by(["date", "unit"])
        .agg([
            pl.col("Số tải kiện đúng").sum().alias("numerator"),
            pl.col("Số tải kiện").sum().alias("denominator"),
        ])
        .pipe(add_date_parts)
        .with_columns([
        ...
        .join(lb.filter(pl.col("status") == "active").select(["unit", "branch"]), on="unit", how="left")
        .select(AGG_COLS)
...
def agg_chonhap(df, lb):
    return (df.group_by(["date","don_vi"])
        ...
        .rename({"don_vi":"unit"}).join(lb.filter(pl.col("status")=="active").select(["unit","branch"]),on="unit",how="left").select(AGG_COLS).filter(pl.col("branch").is_not_null())).filter(~((pl.col("kpi_id")==4)
...
def agg_kndd(df, lb):
    return (df.group_by(["date","Mã đơn vị vận chuyển"])
        ...
        .rename({"Mã đơn vị vận chuyển":"unit"}).join(lb.filter(pl.col("status")=="active").select(["unit","branch"]),on="unit",how="left").select(AGG_COLS).filter(pl.col("branch").is_not_null()))
...
with tab_cfg:
    ...
    with dc1:
        raw_dir = st.text_input("Raw", value=r"D:\0. Data Mega Mart\Raw_KPI",           key="raw_dir")
    with dc2:
    ...
    with sc5:
        sub_15s = st.text_input("12.5s",     value=r"NhapTTKT\T7",          key="sub_15s")
    ...
    # 12.5s — xử lý 1 file lẻ mỗi lần (mặc định file mới nhất), append vào file lũy kế theo tháng
    s15_dir = RAW_P / st.session_state.sub_15s
    with st.expander("📥 Tỷ lệ nhập đúng tải kiện 12.5s — chọn file", expanded=True):
        run_15s = st.checkbox("Bật 12.5s", value=True, key="run_15s")
        if run_15s:
            _s15 = file_dropdown("File 15s (mặc định: mới nhất)", s15_dir, "sel_15s", exts=(".xlsx", ".xlsm", ".csv"))
            if _s15:
                st.caption(f"📅 {datetime.fromtimestamp(_s15.stat().st_mtime).strftime('%d/%m/%Y %H:%M')}")
                # st.caption("File này sẽ được clean gọn lại rồi **append** vào `15s_{tháng}.csv` (tạo mới nếu chưa có).")
...
with tab_run:
    ...
    if run_all:
        ...
        # ── 12.5s — xử lý 1 file lẻ rồi append vào file lũy kế theo tháng ──
        if st.session_state.get("run_15s", True):
            ...
            try:
                ...
                s15_path = sel("sel_15s", s15_dir)
                if not s15_path:
                    raise FileNotFoundError("Chưa chọn file 12.5s")

                log(f"12.5s: xử lý file lẻ {s15_path.name}")
                df_new = clean_15s(s15_path)
                st.session_state.df_clean["15s"] = df_new

                out = CLEAN / "15s"; out.mkdir(parents=True, exist_ok=True)
                out_path = out / f"Nhap15s_{month_label}.csv"
                is_new_file = not out_path.exists()
                df_all = append_clean_15s(df_new, out_path)

                st.session_state.run_status["15s"] = "ok"
                log(
                    f"12.5s CLEAN OK — file mới {df_new.height:,} dòng "
                    f"→ {'tạo mới' if is_new_file else 'append vào'} {out_path.name} "
                    f"(lũy kế {df_all.height:,} dòng)"
                )
            except Exception as e:
        ...
        try:
            ...
            lb_full = pl.read_csv(lk_du).select(["unit","branch","dvc_ic","status"])
            lb = lb_full.select(["unit","branch","status"])
            frames = []
            ...
            if xs_clean_dir.exists():
                files = sorted(xs_clean_dir.glob("*.csv"))
            ...
            s15_clean_path = CLEAN / "15s" / f"Nhap15s_{month_label}.csv"
            if s15_clean_path.exists():
                try:
                    df_15s_all = pl.read_csv(s15_clean_path, try_parse_dates=True).with_columns(pl.col("date").cast(pl.Date))
                    frames.append(agg_15s(df_15s_all, lb))
                    log(f"Agg 12.5s OK — đọc lũy kế {df_15s_all.height:,} dòng từ {s15_clean_path.name}")
                except Exception as ex:
                    log(f"Agg 12.5s SKIP: {ex}", "WARN")
            else:
                log(f"Agg 12.5s SKIP — chưa có file clean: {s15_clean_path}", "WARN")
            if frames:
                df_final = pl.concat(frames, how="vertical_relaxed")


                #=================
                #Lấy thêm dvc_ic để tạo KPI cho đội vận chuyển (đối với mấy KPI bị lẫn)
                try:
                    df_dvc = (
                        df_final
                        .join(lb_full.select(["unit", "dvc_ic"]), on="unit", how="left")
                        .filter(pl.col("dvc_ic").is_not_null() & (pl.col("dvc_ic") != ""))
                        .group_by(["kpi_name", "kpi_id", "dvc_ic", "date", "year", "month", "day"])
                        .agg(
                            pl.col("numerator").sum(),
                            pl.col("denominator").sum(),
                        )
                        .rename({"dvc_ic": "unit"})
                        .join(lb, on="unit", how="left")
                        .select(AGG_COLS)
                        .filter(pl.col("branch").is_not_null())
                    )
                    if df_dvc.height:
                        df_final = pl.concat([df_final, df_dvc], how="vertical_relaxed")
                        log(f"Agg dvc_ic OK — {df_dvc.height:,} rows, {df_dvc['unit'].n_unique()} dvc_ic")
                    else:
                        log("Agg dvc_ic SKIP — không có dvc_ic hợp lệ", "WARN")
                except Exception as ex:
                    log(f"Agg dvc_ic ERROR: {ex}", "ERROR")
...
with tab_flow:
    ...
    flows = [
        ...
        },
        {
            ...
            "steps": [
                ("Input", ["1 file lẻ mỗi lần chạy (mặc định: file mới nhất trong thư mục 15s/), tên dạng *_ddmmyyyy.xlsx"]),
                ("Xử lý", [
                    "Đọc file, filter Chuyến xe ≠ N/A",
                    "Parse Thời gian nhận bàn giao / Thời gian quy định",
                    "Group theo Đơn vị × Chuyến xe × Tuyến xe × Biển kiểm soát → mỗi chuyến xe 1 dòng",
                    "Tính Số tải kiện, Số tải kiện đúng/sai, TG nhập tải đầu/cuối, TG quy định",
                    "Gắn cột date lấy từ tên file",
                ]),
                ("Output", ["Append vào Clean/15s/15s_{month}.csv (tạo mới nếu chưa có; trùng date+chuyến xe thì đè bằng dòng mới)"]),
            ],
            "agg": "Đọc toàn bộ file lũy kế 15s_{month}.csv, nhóm theo date × unit: numerator = Σ Số tải kiện đúng, denominator = Σ Số tải kiện",
            "kpi_id": "kpi_id=5",
        },
    ]
